...
export async function POST(request) {
  try {
    ...
    const [rows] = await pool.execute(
      'INSERT INTO audit_shareholder (create_by, motive, remark, create_time,order_id) VALUES (?, ?, ?, ?, ?)',
    ...
    for (const user of userRowsT1) {
      ...
      await pool.execute(
        'INSERT INTO audit_shareholder_log (audit_id, audit_name, organization, autograph, status, shareholder_id,create_time,remark) VALUES (?, ?, ?, ?, ?, ?,?,?)',
        [
          logData.audit_id,
          logData.audit_name,
