...
export async function POST(request) {
  try {
    ...
    // 更新内容
    const updateFields = ['status = ?', 'update_time = ?', 'update_by = ?']
    const updateValues = [body.status, dayjs().format('YYYY-MM-DD HH:mm:ss'), userId]
    ...
    const sql = `UPDATE audit_trustee SET ${updateFields.join(', ')} WHERE id = ?`
