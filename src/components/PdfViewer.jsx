...
const PdfViewer = ({ url }) => {
  ...
  const handleClick = () => {
    // 优先校验 url
    if (!isValidUrl(url)) {
      Toast.warning({ content: 'PDF 链接无效，无法预览', duration: 3 })
      return
    }
  ...
  return (
    <>
      {isValidUrl(url) ? (
        <svg className="cursor-pointer" viewBox="0 0 1024 1024" width="30" height="30" onClick={() => handleClick()}>
          <path
            fill="#D93838"
      ...
      <Modal centered title="预览PDF" width={'80%'} visible={visible} onCancel={() => setVisible(false)} footer={null}>
        <iframe src={url} className="w-full h-[70vh] pb-5" title="预览PDF" />
      </Modal>
    </>
  )
}
